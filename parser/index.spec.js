...
describe('Parser', () => {
  ...
  it('renders markdown', async () => {
    expect.assertions(1)
    const actual = await parser('*Hello* **[world](https://thefifthworld.com)**', null, db)
  ...
  it('doesn\'t parse tags that are inside code blocks', async () => {
    expect.assertions(1)
    const actual = await parser('```\n[[Test:Hello]]\n```\n\nThis is outside of the code block.', null, db)
  ...
  it('parses links', async () => {
    ...
    const actual = await parser('[[Test Page | Hello]]', null, db)
  ...
  it('parses templates', async () => {
    ...
    const actual = await parser('{{Test}}', null, db)
    await testUtils.resetTables(db)
